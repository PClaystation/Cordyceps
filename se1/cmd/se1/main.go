...
func main() {
	...
	flag.BoolVar(&backgroundFlag, "background", resolveBoolSetting("SE1_BACKGROUND_MODE", defaultBackgroundMode), "Run detached in the background on Windows")
	flag.BoolVar(&startupFlag, "startup", resolveBoolSetting("SE1_INSTALL_STARTUP", defaultStartupMode), "Register the agent to start automatically on Windows")
