...
function Resolve-AgentExePath([string]$requestedPath, [string]$scriptRoot, [string]$defaultExeName) {
  ...
  throw "Agent executable not found. Build or copy ds1-agent.exe (or dist\\ds1-agent-usb.exe) first."
}

$scriptRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$resolvedAgentExePath = Resolve-AgentExePath -requestedPath $AgentExePath -scriptRoot $scriptRoot -defaultExeName "ds1-agent.exe"
...
Write-Host "Starting DS1 agent enrollment..."
...
Write-Host "If DeviceId was omitted, the server auto-designates (for example: ds1, ds2, ...)."
Write-Host "Config path: $env:APPDATA\DS1Agent\config.json"
