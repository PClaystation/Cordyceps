param(
  ...
  [string]$OutputPath = ".\dist\ds1-agent-usb.exe",
...
try {
  $resourceState = New-CordycepsWindowsBuildResource `
    -RepoRoot $repoRoot `
    -PackageDir (Join-Path $scriptRoot "cmd/ds1") `
    -Version $Version `
    -ProductName "Cordyceps DS1 Agent" `
    -FileDescription "Cordyceps DS1 USB-ready Windows resident agent" `
    -OriginalFilename (Split-Path -Leaf $outputFullPath) `
    -InternalName "ds1-agent"
