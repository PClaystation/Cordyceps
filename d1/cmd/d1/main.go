...
import (
	...
	"github.com/charliearnerstal/cordyceps/d1/internal/commands"
	...
	"github.com/charliearnerstal/cordyceps/d1/internal/updater"
...
func main() {
	...
	if opts.printVersion {
		fmt.Println(opts.effectiveVersion())
		return
	}

	log.SetFlags(log.LstdFlags | log.LUTC)

	if handled, err := handleWindowsService(opts); handled {
		if err != nil {
			log.Fatal(err)
		}
		return
	...
	configureLogging(opts.foreground, opts.enrollOnly)

	if err := runStandaloneAgent(opts); err != nil {
		log.Fatal(err)
	}
}

func parseAgentOptions() agentOptions {
	var opts agentOptions

	flag.StringVar(&opts.serverURL, "server-url", resolveStringSetting("CORDYCEPS_SERVER_URL", defaultServerURL), "Server base URL (e.g. https://cordyceps.example)")
	flag.StringVar(&opts.deviceID, "device-id", "", "Device ID (e.g. d1)")
	flag.StringVar(&opts.displayName, "display-name", resolveStringSetting("CORDYCEPS_DISPLAY_NAME", ""), "Shared display name shown by remotes for this device")
	flag.StringVar(&opts.bootstrapToken, "bootstrap-token", resolveStringSetting("CORDYCEPS_BOOTSTRAP_TOKEN", defaultBootstrapToken), "Bootstrap token for first-run enrollment")
	flag.StringVar(&opts.version, "version", defaultVersion, "Agent version string")
	flag.StringVar(&opts.configPath, "config", "", "Path to agent config file")
	...
	flag.StringVar(&opts.slotName, "slot", "", "Internal active slot name")
	flag.BoolVar(&opts.background, "background", resolveBoolSetting("CORDYCEPS_BACKGROUND_MODE", defaultBackgroundMode), "Run detached in the background on Windows")
	flag.BoolVar(&opts.startup, "startup", resolveBoolSetting("CORDYCEPS_INSTALL_STARTUP", defaultStartupMode), "Register the agent to start automatically on Windows")
	flag.BoolVar(&opts.enrollOnly, "enroll-only", false, "Enroll and exit")
	flag.BoolVar(&opts.foreground, "foreground", false, "Run in the current console instead of background mode (Windows)")
	flag.BoolVar(&opts.runAgent, "run-agent", false, "Internal flag used for detached relaunch")
	flag.BoolVar(&opts.printVersion, "print-version", false, "Print effective version and exit")
	flag.BoolVar(&opts.installService, "install", false, "Install d1 as an automatic Windows service")
	flag.BoolVar(&opts.uninstallSvc, "uninstall", false, "Uninstall the d1 Windows service")
	flag.BoolVar(&opts.interactiveSvc, "interactive", false, "Run the Windows service handler interactively for testing")
	flag.Parse()
	...
	opts.slotName = resilience.NormalizeSlot(opts.slotName)

	return opts
}

func (opts agentOptions) effectiveVersion() string {
	if opts.version != "" {
		return opts.version
	}
...
func runStandaloneAgent(opts agentOptions) error {
	...
	if executableResolved {
		if installed, err := installAndRelaunchIfNeeded(executablePath, os.Args[1:], opts.foreground, opts.runAgent, opts.enrollOnly, paths); err != nil {
	...
	return runAgentHost(ctx, opts, paths, executablePath, executableResolved, true)
}

func runAgentHost(ctx context.Context, opts agentOptions, paths resilience.Paths, executablePath string, executableResolved bool, allowStartup bool) error {
	...
	if slotName == "" {
		slotName = resilience.InferSlotFromExecutable(executablePath, paths)
	}
	slotName = resilience.NormalizeSlot(slotName)

	if opts.enrollOnly {
		if _, err := initializeAgent(cfgPath, opts.serverURL, opts.deviceID, opts.displayName, opts.bootstrapToken, opts.effectiveVersion(), opts.versionExplicit, executablePath, ensureStartup); err != nil {
	...
	superviseAgent(
		...
		opts.displayName,
		opts.bootstrapToken,
		opts.effectiveVersion(),
		opts.versionExplicit,
		executablePath,
		ensureStartup,
		paths,
		slotName,
...
func currentExecutablePath() (string, bool) {
...
func firstRunEnroll(cfgPath string, serverBaseURL string, deviceIDInput string, displayNameInput string, bootstrapToken string, version string) (*config.Config, error) {
	...
	if autoDesignate {
		deviceID = ""
	} else if !strings.HasPrefix(deviceID, "d") {
		deviceID = config.SanitizeDeviceID("d-" + deviceID)
	}
...
func initializeAgent(cfgPath string, serverURL string, deviceID string, displayName string, bootstrapToken string, version string, versionExplicit bool, executablePath string, ensureStartup bool) (*config.Config, error) {
	...
	if ensureStartup {
		...
		if err := repairWindowsServiceIfInstalled(); err != nil {
			log.Printf("warning: Windows service repair failed: %v", err)
		}
	}
...
func superviseAgent(
	...
	for {
		...
		backoff = 2 * time.Second
		if err := runLoop(ctx, cfg, cfgPath, paths, slotName); err != nil {
...
func runLoop(ctx context.Context, cfg *config.Config, cfgPath string, paths resilience.Paths, slotName string) error {
	backoff := 2 * time.Second
	...
	for {
		...
		err := safeRunSession(ctx, cfg, cfgPath, paths, slotName)
		if err == nil {
			return nil
		}
...
func runSession(ctx context.Context, cfg *config.Config, cfgPath string, paths resilience.Paths, slotName string) error {
		...
		for {
			...
			case "command":
				...
				if strings.EqualFold(strings.TrimSpace(command.Type), "AGENT_UPDATE") {
					updateResult, restartRequired := executeUpdateCommand(cfg, cfgPath, paths, slotName, command)
...
func executeUpdateCommand(
	...
	applyResult, err := updater.Apply(command.Args, executablePath, cfgPath, paths, slotName)
	if err != nil {
		result.OK = false
		result.ErrorCode = "UPDATE_FAILED"
...
func safeRunSession(ctx context.Context, cfg *config.Config, cfgPath string, paths resilience.Paths, slotName string) (err error) {
	defer func() {
	...
	return runSession(ctx, cfg, cfgPath, paths, slotName)
}
...
func installAndRelaunchIfNeeded(
	executablePath string,
	args []string,
	foreground bool,
	runAgent bool,
	...
	installedPath := resilience.SlotExecutablePath(paths, resilience.SlotA)
...
func maintainStartupRegistration(ctx context.Context, executablePath string) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			if err := startup.RepairStartupRegistrationIfMissing(executablePath); err != nil {
				log.Printf("warning: periodic startup integrity repair failed: %v", err)
			}
			if err := repairWindowsServiceIfInstalled(); err != nil {
				log.Printf("warning: periodic Windows service repair failed: %v", err)
			}
		}
	}
}

func markHealthy(paths resilience.Paths, slotName string, cfg *config.Config) error {
	return resilience.SaveHealth(paths, &resilience.Health{
		DeviceID: strings.TrimSpace(cfg.DeviceID),
		Slot:     resilience.NormalizeSlot(slotName),
