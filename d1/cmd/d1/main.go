...
import (
	...
	"github.com/charliearnerstal/jarvis/d1/internal/commands"
	...
	"github.com/charliearnerstal/jarvis/d1/internal/updater"
...
func parseAgentOptions() agentOptions {
	...
	flag.StringVar(&opts.serverURL, "server-url", resolveStringSetting("JARVIS_SERVER_URL", defaultServerURL), "Server base URL (e.g. https://jarvis.example)")
	flag.StringVar(&opts.deviceID, "device-id", "", "Device ID (e.g. d1)")
	flag.StringVar(&opts.displayName, "display-name", strings.TrimSpace(os.Getenv("JARVIS_DISPLAY_NAME")), "Shared display name shown by remotes for this device")
	flag.StringVar(&opts.bootstrapToken, "bootstrap-token", resolveStringSetting("JARVIS_BOOTSTRAP_TOKEN", defaultBootstrapToken), "Bootstrap token for first-run enrollment")
	...
	flag.BoolVar(&opts.background, "background", resolveBoolSetting("JARVIS_BACKGROUND_MODE", defaultBackgroundMode), "Run detached in the background on Windows")
	flag.BoolVar(&opts.startup, "startup", resolveBoolSetting("JARVIS_INSTALL_STARTUP", defaultStartupMode), "Register the agent to start automatically on Windows")
