...
const (
	d1ServiceName        = "CordycepsD1"
	d1ServiceDisplayName = "Cordyceps d-family Helper (d1)"
	d1ServiceDescription = "Runs the personal d-family development helper for d1 as a Windows service."
...
func (service *d1WindowsService) Execute(args []string, requests <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	service.info(eventIDServiceStarting, "d1 Windows service is starting")

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	executablePath, executableResolved := currentExecutablePath()
	paths, err := resilience.ResolvePaths(service.opts.configPath, service.opts.installRoot, service.opts.programDataRoot)
	if err != nil {
		service.error(eventIDServiceError, fmt.Sprintf("resolve runtime paths failed: %v", err))
		return false, 1
	}
	agentDone := make(chan error, 1)
	go func() {
		agentDone <- runAgentHost(ctx, service.opts, paths, executablePath, executableResolved, false)
	}()

	changes <- svc.Status{State: svc.Running, Accepts: acceptedCommands}
	service.info(eventIDServiceRunning, "d1 Windows service is running")
	...
	for {
		...
		case change := <-requests:
			...
			case svc.Stop, svc.Shutdown:
				service.info(eventIDServiceStopping, "d1 Windows service stop requested")
				changes <- svc.Status{State: svc.StopPending}
				cancel()

				if err := <-agentDone; err != nil {
					service.error(eventIDServiceError, fmt.Sprintf("d1 Windows service stopped with error: %v", err))
					return false, 1
				}

				service.info(eventIDServiceStopped, "d1 Windows service stopped cleanly")
				return false, 0
			default:
		...
		case err := <-agentDone:
			changes <- svc.Status{State: svc.StopPending}
			if err != nil {
				service.error(eventIDServiceError, fmt.Sprintf("d1 agent loop exited with error: %v", err))
				return false, 1
			}

			service.warning(eventIDServiceWarning, "d1 agent loop exited; stopping Windows service")
			return false, 0
		}
	}
}
...
func serviceInstallArgs(opts agentOptions) []string {
	...
	if opts.slotName != "" {
		args = append(args, "--slot", opts.slotName)
	}
	if opts.serverURL != "" {
